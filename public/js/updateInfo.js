...
if (localStorageSaved == null) {
  ...
  db.collection('user').doc(JSON.parse(localStorageSaved).uid).get().then((user) => {
    const nickName = user.data().name
    const stuName = user.data().stuName
    const email = user.data().email
    const deleteAgree = user.data().deleteAgree
    const yearOfAdmission = user.data().yearOfAdmission
    const realName = user.data().realName


    $('#stuName').val(stuName)
    $('#nickName').val(nickName)
    $('#email-new').val(email)
    $("#birth-year").val(yearOfAdmission)
    $("#my-name").val(realName)
  })
  ...
  $('#update').click(function(){
    ...
    let userProvidedPassword = $('#pw-new').val();
    ...
    if (yearOfAdmissionVal == null) {
      ...
      user.updateEmail($('#email-new').val()).then(() => {
        user.updateProfile({
          displayName: nickName,
          }).then(() => {
            let updateProfileVal = {
              nickName : nickName,
              email : email,
            ...
            db.collection('user').doc(JSON.parse(localStorageSaved).uid).update(updateProfileVal).then(() => {
